  ...
  return (
    <div style={{ marginBottom: '1.25rem' }}>
      ...
      <div onDragOver={e => { e.preventDefault(); setDrag(true) }} onDragLeave={() => setDrag(false)} onDrop={handleDrop}
        ...
        style={{ border: `2px dashed ${drag ? '#0F2347' : file ? '#1A7A4A' : '#E5E7EB'}`, borderRadius: 8, padding: '1.5rem', textAlign: 'center', cursor: 'pointer', backgroundColor: drag ? 'rgba(15,35,71,0.03)' : file ? '#F0FAF4' : '#F9FAFB' }}>
        ...
        {file ? (
          <div>
            ...
            <div style={{ fontSize: '0.85rem', color: '#1A7A4A', fontFamily: 'Helvetica Neue, Arial, sans-serif', fontWeight: 600 }}>
              {multiple ? `${file.length} fichier(s) selectionne(s)` : file.name}
            </div>
          </div>
        ) : (
          <div>
            ...
            <div style={{ fontSize: '0.85rem', color: '#6B7280', fontFamily: 'Helvetica Neue, Arial, sans-serif' }}>
              Glissez-deposez ou <span style={{ color: '#0F2347', fontWeight: 600 }}>parcourir</span>
            </div>
...
export default function NewClaim() {
  ...
  const [form, setForm] = useState({ equipmentId: '', incidentDate: '', description: '', amount: '', location: '' })
  ...
  useEffect(() => {
    api.get('/equipment')
      .then(res => {
        ...
        setEquipmentList(Array.isArray(d) ? d : [])
      })
      .catch(() => {})
  ...
  const handleNext = () => {
    ...
    if (step === 0) {
      ...
      if (form.description.length < 20) {
        setError('La description doit contenir au moins 20 caracteres.'); return
      }
      if (isNaN(form.amount) || Number(form.amount) <= 0) {
        setError('Le montant doit etre un nombre positif.'); return
      }
    }
    if (step === 1) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Tous les fichiers sous le champ "files" (nom defini dans le backend)
  ...
  const inputStyle = { width: '100%', padding: '0.72rem 0.9rem', border: '1.5px solid #E5E7EB', borderRadius: 6, fontSize: '0.9rem', fontFamily: 'Helvetica Neue, Arial, sans-serif', outline: 'none', backgroundColor: '#F9FAFB', boxSizing: 'border-box' }
  const labelStyle = { display: 'block', fontSize: '0.74rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', color: '#4B5563', marginBottom: '0.4rem', fontFamily: 'Helvetica Neue, Arial, sans-serif' }
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', backgroundColor: pageBg, fontFamily: 'Georgia, serif' }}>
      ...
      <div style={{ marginLeft: 240, flex: 1, padding: '2rem' }}>
        ...
        <div style={{ maxWidth: 720, backgroundColor: cardBg, borderRadius: 12, border: `1px solid ${cardBorder}`, padding: '2rem' }}>
          {step === 3 ? (
            <div style={{ textAlign: 'center', padding: '2rem 0' }}>
              ...
              <h2 style={{ color: textMain, fontSize: '1.5rem', fontWeight: 400, marginBottom: '0.5rem' }}>Sinistre <strong>soumis avec succes</strong></h2>
              <p style={{ color: textSub, fontFamily: 'Helvetica Neue, Arial, sans-serif', fontSize: '0.9rem', marginBottom: '1.5rem' }}>Votre dossier est en cours d'analyse par notre systeme IA</p>
              <div style={{ backgroundColor: pageBg, border: `1px solid ${cardBorder}`, borderRadius: 8, padding: '1rem 2rem', display: 'inline-block', marginBottom: '2rem' }}>
                <div style={{ fontSize: '0.72rem', textTransform: 'uppercase', color: textSub, fontFamily: 'Helvetica Neue, Arial, sans-serif', marginBottom: '0.25rem' }}>Reference</div>
                <div style={{ fontSize: '1.4rem', fontWeight: 700, color: '#C9A84C', fontFamily: 'Helvetica Neue, Arial, sans-serif' }}>{claimRef}</div>
              </div>
              <div style={{ backgroundColor: '#FEF9E7', border: '1px solid #F7DC6F', borderRadius: 8, padding: '0.75rem', marginBottom: '2rem', fontSize: '0.85rem', color: '#7D6608', fontFamily: 'Helvetica Neue, Arial, sans-serif' }}>
                Temps d'analyse estime : moins de 5 minutes
              </div>
              <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center' }}>
                <button onClick={() => navigate('/client/dashboard')} style={{ padding: '0.75rem 1.5rem', border: `1.5px solid ${cardBorder}`, borderRadius: 8, fontSize: '0.85rem', fontFamily: 'Helvetica Neue, Arial, sans-serif', cursor: 'pointer', background: cardBg, color: textSub }}>Retour au dashboard</button>
                <button onClick={() => { setStep(0); setForm({ equipmentId: '', incidentDate: '', description: '', amount: '', location: '' }); setFiles({ csv: null, photos: null, pdf: null }) }}
                  style={{ padding: '0.75rem 1.5rem', background: 'linear-gradient(135deg, #0F2347, #1A3A6B)', color: 'white', border: 'none', borderRadius: 8, fontSize: '0.85rem', fontFamily: 'Helvetica Neue, Arial, sans-serif', cursor: 'pointer', fontWeight: 600 }}>Nouveau sinistre</button>
              </div>
            </div>
          ) : (
            <>
              <StepIndicator current={step} />
              {error && <div style={{ backgroundColor: '#FDF2F2', border: '1px solid #EBCECE', borderRadius: 6, padding: '0.7rem 0.9rem', color: '#C0392B', fontSize: '0.82rem', fontFamily: 'Helvetica Neue, Arial, sans-serif', marginBottom: '1.5rem' }}>⚠ {error}</div>}
              ...
              {step === 0 && (
                <div>
                  ...
                  <div style={{ marginBottom: '1rem' }}>
                    <label style={labelStyle}>Equipement concerne <span style={{ color: '#C0392B' }}>*</span></label>
                    {equipmentLoading ? (
                      <div style={{ padding: '0.72rem', color: textSub, fontFamily: 'Helvetica Neue, Arial, sans-serif' }}>Chargement...</div>
                    ) : equipmentList.length === 0 ? (
                      <div style={{ padding: '0.72rem', backgroundColor: '#FEF9E7', border: '1px solid #F7DC6F', borderRadius: 6, color: '#7D6608', fontFamily: 'Helvetica Neue, Arial, sans-serif', fontSize: '0.85rem' }}>
                        Aucun equipement enregistre. Ajoutez-en un dans votre profil.
                      </div>
                    ) : (
                      <select value={form.equipmentId} onChange={e => setForm({ ...form, equipmentId: e.target.value })} style={inputStyle}>
                        ...
                        {equipmentList.map(eq => <option key={eq.id} value={eq.id}>{eq.name}{eq.serialNumber ? ` — ${eq.serialNumber}` : ''}</option>)}
                      </select>
                    )}
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
                    <div>
                      ...
                      <input type="date" value={form.incidentDate} max={new Date().toISOString().split('T')[0]} onChange={e => setForm({ ...form, incidentDate: e.target.value })} style={inputStyle} />
                    </div>
                    <div>
                      <label style={labelStyle}>Lieu</label>
                      <input type="text" placeholder="Ex: Usine principale, Zone B" value={form.location} onChange={e => setForm({ ...form, location: e.target.value })} style={inputStyle} />
                    </div>
                  </div>
                  <div style={{ marginBottom: '1rem' }}>
                    <label style={labelStyle}>Montant reclame (DZD) <span style={{ color: '#C0392B' }}>*</span></label>
                    <input type="number" placeholder="Ex: 850000" value={form.amount} onChange={e => setForm({ ...form, amount: e.target.value })} style={inputStyle} />
                  </div>
                  <div style={{ marginBottom: '1.5rem' }}>
                    <label style={labelStyle}>Description <span style={{ color: '#C0392B' }}>*</span></label>
                    <textarea rows={4} placeholder="Decrivez en detail ce qui s'est passe (minimum 20 caracteres)..." value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} style={{ ...inputStyle, resize: 'vertical' }} />
                    <div style={{ fontSize: '0.72rem', color: form.description.length >= 20 ? '#1A7A4A' : '#C0392B', fontFamily: 'Helvetica Neue, Arial, sans-serif', marginTop: '0.25rem' }}>{form.description.length}/20 caracteres minimum</div>
                  </div>
                </div>
              )}
              ...
              {step === 1 && (
                <div>
                  ...
                  <p style={{ color: textSub, fontSize: '0.85rem', fontFamily: 'Helvetica Neue, Arial, sans-serif', marginBottom: '1.5rem' }}>Ces fichiers seront analyses par nos modeles IA</p>
                  <FileUploadZone label="Donnees capteurs CSV" accept=".csv" required={true} hint="Fichier CSV avec donnees capteurs (min 6 mois)" file={files.csv} onChange={f => setFiles({ ...files, csv: f })} />
                  <FileUploadZone label="Photos equipement" accept="image/*" required={true} multiple={true} hint="JPG, PNG — 1 a 10 photos" file={files.photos} onChange={f => setFiles({ ...files, photos: f })} />
                  <FileUploadZone label="Rapport technique PDF" accept=".pdf" required={false} hint="Optionnel — rapport d'expertise" file={files.pdf} onChange={f => setFiles({ ...files, pdf: f })} />
                </div>
              )}
              ...
              {step === 2 && (
                <div>
                  ...
                  <div style={{ backgroundColor: pageBg, borderRadius: 8, border: `1px solid ${cardBorder}`, overflow: 'hidden', marginBottom: '1.5rem' }}>
                    <div style={{ padding: '0.75rem 1rem', backgroundColor: '#0F2347' }}>
                      <span style={{ color: 'white', fontSize: '0.82rem', fontWeight: 600, fontFamily: 'Helvetica Neue, Arial, sans-serif' }}>Recapitulatif</span>
                    </div>
                    {[
                      ['Equipement',    selectedEquipment?.name || '-'],
                      ['Date incident', form.incidentDate ? new Date(form.incidentDate).toLocaleDateString('fr-FR') : '-'],
                      ['Lieu',          form.location || 'Non specifie'],
                      ['Montant',       `${Number(form.amount).toLocaleString('fr-FR')} DZD`],
                  ...
                  <div style={{ backgroundColor: '#FEF9E7', border: '1px solid #F7DC6F', borderRadius: 8, padding: '0.75rem 1rem', fontSize: '0.82rem', color: '#7D6608', fontFamily: 'Helvetica Neue, Arial, sans-serif' }}>
                    En soumettant, vous certifiez que toutes les informations sont exactes et veridiques.
                  </div>
                </div>
              )}

              <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '1.5rem', paddingTop: '1.5rem', borderTop: `1px solid ${cardBorder}` }}>
                <button onClick={() => step === 0 ? navigate('/client/dashboard') : setStep(s => s - 1)}
                ...
                {step < 2 ? (
                  <button onClick={handleNext} style={{ padding: '0.75rem 1.5rem', background: 'linear-gradient(135deg, #0F2347, #1A3A6B)', color: 'white', border: 'none', borderRadius: 8, fontSize: '0.85rem', fontFamily: 'Helvetica Neue, Arial, sans-serif', cursor: 'pointer', fontWeight: 600 }}>Suivant</button>
                ) : (
                  <button onClick={handleSubmit} disabled={loading}
                    ...
                    {loading ? 'Soumission en cours...' : 'Soumettre le sinistre'}
                  </button>
                )}
              </div>
            </>
