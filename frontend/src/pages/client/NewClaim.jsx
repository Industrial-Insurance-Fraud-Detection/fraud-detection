 ...
 * Body fields the backend expects:
 *   equipmentId     string
 *   incidentDate    string  YYYY-MM-DD
 *   description     string  min 20 chars
 *   claimedAmount   number  (sent as string in FormData, backend @Type(() => Number) converts)
 *   files           File[]  field name must be "files" — FilesInterceptor('files', 20)
 *
 * Response: { success, data: { claimId, reference, status, filesUploaded, ... } }
 *
 * GET /equipment  →  { success, data: { data: Equipment[], pagination } }
 */
...
export default function NewClaim() {
  ...
  // Fetch equipment list — unwrap paginated response
  useEffect(() => {
    api.get('/equipment')
      .then(res => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      formData.append('claimedAmount', form.amount)           // backend @Type(() => Number) handles it

      // All files under the SAME field name "files"
      // Backend: FilesInterceptor('files', 20) — matches exactly
      ...
      // { success, data: { claimId, reference, status, filesUploaded, ... } }
            ...
            <div style={{ textAlign: 'center', padding: '2rem 0' }}>
              ...
              <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center' }}>
                <button
                  ...
                  style={{ padding: '0.75rem 1.5rem', background: 'linear-gradient(135deg, #0F2347, #1A3A6B)', color: 'white', border: 'none', borderRadius: 8, fontSize: '0.85rem', fontFamily: 'Helvetica Neue, Arial, sans-serif', cursor: 'pointer', fontWeight: 600 }}>
                  Voir le dossier
                </button>
                <button
