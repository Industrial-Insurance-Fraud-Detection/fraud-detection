...
/**
 * FIXED: correct unwrapping of paginated response.
 * Backend chain: TransformInterceptor → { success, data: paginate() }
 * paginate() → { data: Claim[], pagination }
 * So full shape: res.data = { success, data: { data: [], pagination } }
 */
function extractClaims(responseData) {
  const inner = responseData?.data ?? responseData   // strip TransformInterceptor
  const arr = inner?.data ?? inner                 // strip paginate()
...
export default function ClientStats() {
  ...
  useEffect(() => {
    api.get('/claims/my')
      .then(res => setClaims(extractClaims(res.data)))   // FIXED: was extractArray(res.data)
  ...
  // --- derive stats from real claim fields ---
  const stats = {
    total: claims.length,
    approved: claims.filter(c => c.status === 'APPROVED').length,
    rejected: claims.filter(c => c.status === 'REJECTED').length,
    // analysis is nested: claim.analysis?.finalScore
    avgScore: (() => {
    ...
    // claimedAmount is the backend field name
  ...
  // score distribution uses claim.analysis?.finalScore
  const scoreData = [
    { range: '0-20', count: claims.filter(c => (c.analysis?.finalScore ?? 50) <= 20).length, color: '#1A7A4A' },
    { range: '21-40', count: claims.filter(c => (c.analysis?.finalScore ?? 50) > 20 && (c.analysis?.finalScore ?? 50) <= 40).length, color: '#27AE60' },
    { range: '41-60', count: claims.filter(c => (c.analysis?.finalScore ?? 50) > 40 && (c.analysis?.finalScore ?? 50) <= 60).length, color: '#F39C12' },
    { range: '61-80', count: claims.filter(c => (c.analysis?.finalScore ?? 50) > 60 && (c.analysis?.finalScore ?? 50) <= 80).length, color: '#E67E22' },
    { range: '81-100', count: claims.filter(c => (c.analysis?.finalScore ?? 50) > 80).length, color: '#C0392B' },
  ]

  // monthly amounts use claimedAmount (not amount)
  const monthData = claims.reduce((acc, c) => {
    ...
    if (existing) {
      existing.montant += c.claimedAmount || 0   // FIXED: was c.amount
      existing.sinistres += 1
    } else {
      acc.push({ month, montant: c.claimedAmount || 0, sinistres: 1 })
    }
    return acc
  }, []).slice(-6)

  // score evolution uses claim.analysis?.finalScore
          ...
          <>
            ...
            <div style={{ display: 'flex', gap: '1rem', marginBottom: '2rem' }}>
              {[
                ...
                {
                  label: 'Score moyen IA', value: `${stats.avgScore}/100`,
                ...
                <div key={s.label}
                  style={{ backgroundColor: cardBg, borderRadius: 12, padding: '1.25rem', border: `1px solid ${cardBorder}`, flex: 1, transition: 'transform 0.18s' }}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', marginBottom: '1.5rem' }}>
              <div style={{ backgroundColor: cardBg, borderRadius: 14, border: `1px solid ${cardBorder}`, padding: '1.5rem' }}>
                <h3 style={{ color: textMain, fontSize: '0.95rem', fontWeight: 600, fontFamily: 'Helvetica Neue, Arial, sans-serif', marginBottom: '1.5rem' }}>Montants declares par periode</h3>
                {monthData.length > 0 ? (
                  <ResponsiveContainer width="100%" height={220}>
                  ...
                  <div style={{ height: 220, display: 'flex', alignItems: 'center', justifyContent: 'center', color: textSub, fontFamily: 'Helvetica Neue, Arial, sans-serif', fontSize: '0.85rem' }}>Pas assez de donnees</div>
                )}
              </div>

              <div style={{ backgroundColor: cardBg, borderRadius: 14, border: `1px solid ${cardBorder}`, padding: '1.5rem' }}>
                <h3 style={{ color: textMain, fontSize: '0.95rem', fontWeight: 600, fontFamily: 'Helvetica Neue, Arial, sans-serif', marginBottom: '1.5rem' }}>Evolution des scores IA</h3>
