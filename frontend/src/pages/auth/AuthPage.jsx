...
export default function AuthPage() {
  ...
  const handleLogin = async (e) => {
	...
	setAuth(user, accessToken, refreshToken)  // ✅ refreshToken correct
    ...
    <div style={{ minHeight: '100vh', display: 'flex', fontFamily: 'Georgia, serif' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', zIndex: 1 }}>
          <div style={{ width: 42, height: 42, borderRadius: 8, background: 'linear-gradient(135deg, #C9A84C, #E8C97A)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20 }}>
            🛡
          </div>
          <div>
      ...
      {/* PANNEAU DROIT */}
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '3rem 4rem', backgroundColor: 'white', position: 'relative' }}>
        ...
        <div style={{ maxWidth: 420, width: '100%', margin: '0 auto' }}>
          ...
          <p style={{ color: '#9CA3AF', fontSize: '0.86rem', marginBottom: '1.5rem', fontFamily: 'Helvetica Neue, Arial, sans-serif', lineHeight: 1.5 }}>
            {tab === 'login' ? 'Acces securise a votre tableau de bord de sinistres.' : 'Creez votre compte pour soumettre et suivre vos sinistres.'}
          </p>
            ...
            <form onSubmit={handleLogin}>
              {[
                ...
                <div key={f.name} style={{ marginBottom: '1rem' }}>
                  <label style={{ display: 'block', fontSize: '0.74rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', color: '#4B5563', marginBottom: '0.4rem', fontFamily: 'Helvetica Neue, Arial, sans-serif' }}>{f.label}</label>
                  <input type={f.type} placeholder={f.placeholder} value={loginForm[f.name]}
                    onChange={e => setLoginForm({ ...loginForm, [f.name]: e.target.value })}
                    style={{ width: '100%', padding: '0.72rem 0.9rem', border: '1.5px solid #E5E7EB', borderRadius: 6, fontSize: '0.9rem', fontFamily: 'Helvetica Neue, Arial, sans-serif', outline: 'none', backgroundColor: '#F9FAFB', boxSizing: 'border-box' }}
                    onFocus={e => e.target.style.borderColor = '#0F2347'}
                    onBlur={e => e.target.style.borderColor = '#E5E7EB'} />
                </div>
              ))}
              <button type="submit" disabled={loading} style={{ width: '100%', padding: '0.85rem', background: 'linear-gradient(135deg, #0F2347, #1A3A6B)', color: 'white', border: 'none', borderRadius: 6, fontSize: '0.86rem', fontFamily: 'Helvetica Neue, Arial, sans-serif', fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? 0.7 : 1, boxShadow: '0 4px 15px rgba(15,35,71,0.25)', marginTop: '0.5rem' }}>
          ...
          {tab === 'register' && (
            <form onSubmit={handleRegister}>
              {/* Choix du role */}
              <label style={{ display: 'block', fontSize: '0.74rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', color: '#4B5563', marginBottom: '0.5rem', fontFamily: 'Helvetica Neue, Arial, sans-serif' }}>Type de compte</label>
              <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '1rem' }}>
                {[
                  { key: 'CLIENT', label: 'Client', desc: 'Entreprise industrielle' },
                  { key: 'INVESTIGATOR', label: 'Enqueteur', desc: "Compagnie d'assurance" },
                ].map(r => (
                  <div key={r.key} onClick={() => setRole(r.key)} style={{ flex: 1, padding: '0.8rem', cursor: 'pointer', textAlign: 'center', border: `1.5px solid ${role === r.key ? '#0F2347' : '#E5E7EB'}`, borderRadius: 8, backgroundColor: role === r.key ? 'rgba(15,35,71,0.04)' : '#F9FAFB', transition: 'all 0.2s' }}>
                    <div style={{ fontSize: '0.8rem', fontWeight: 600, color: role === r.key ? '#0F2347' : '#6B7280', fontFamily: 'Helvetica Neue, Arial, sans-serif' }}>{r.label}</div>
                    <div style={{ fontSize: '0.7rem', color: '#9CA3AF', fontFamily: 'Helvetica Neue, Arial, sans-serif', marginTop: 2 }}>{r.desc}</div>
                  </div>
                ))}
              </div>

              {/* Prenom + Nom sur la meme ligne */}
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem', marginBottom: '0.8rem' }}>
                <div>
                  <label style={{ display: 'block', fontSize: '0.74rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', color: '#4B5563', marginBottom: '0.35rem', fontFamily: 'Helvetica Neue, Arial, sans-serif' }}>Prenom *</label>
                ...
                <div>
                  <label style={{ display: 'block', fontSize: '0.74rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', color: '#4B5563', marginBottom: '0.35rem', fontFamily: 'Helvetica Neue, Arial, sans-serif' }}>Nom *</label>
              ...
              {[
                { label: 'Entreprise', name: 'company', type: 'text', placeholder: 'Nom de votre entreprise' },
                { label: 'Adresse email *', name: 'email', type: 'email', placeholder: 'votre@email.com' },
                ...
                <div key={f.name} style={{ marginBottom: '0.8rem' }}>
                  <label style={{ display: 'block', fontSize: '0.74rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', color: '#4B5563', marginBottom: '0.35rem', fontFamily: 'Helvetica Neue, Arial, sans-serif' }}>{f.label}</label>
              ...
              <button type="submit" disabled={loading} style={{ width: '100%', padding: '0.85rem', marginTop: '0.5rem', background: 'linear-gradient(135deg, #0F2347, #1A3A6B)', color: 'white', border: 'none', borderRadius: 6, fontSize: '0.86rem', fontFamily: 'Helvetica Neue, Arial, sans-serif', fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? 0.7 : 1, boxShadow: '0 4px 15px rgba(15,35,71,0.25)' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: '1.5rem', marginTop: '1.5rem', paddingTop: '1.25rem', borderTop: '1px solid #F3F4F6' }}>
            {['JWT Securise', 'HTTPS', 'bcrypt'].map(label => (
              <div key={label} style={{ color: '#9CA3AF', fontSize: '0.72rem', fontFamily: 'Helvetica Neue, Arial, sans-serif' }}>
                {label}
              </div>
            ))}
          </div>
