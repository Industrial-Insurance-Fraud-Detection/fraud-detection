...
export default function InvestigatorDashboard() {
  ...
  const filtered = claims
    .filter(c => {
      const priority    = getPriority(c.finalScore || 50)
  ...
  const stats = {
    total:       claims.length,
    urgent:      claims.filter(c => getPriority(c.finalScore || 50) === 'HIGH').length,
    avgScore:    claims.length > 0 ? Math.round(claims.reduce((s, c) => s + (c.finalScore || 50), 0) / claims.length) : 0,
      ...
      <div style={{ marginLeft: 240, flex: 1, padding: '2rem', transition: 'margin 0.25s' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '2rem' }}>
          <div>
            ...
            <h1 style={{ fontSize: '1.9rem', color: textMain, fontWeight: 400 }}>
              Bonjour, <strong>{user?.fullName?.split(' ')[0] || 'Investigateur'}</strong> 🔍
            </h1>
        ...
        <div style={{ backgroundColor: cardBg, borderRadius: 14, border: `1px solid ${cardBorder}`, overflow: 'hidden' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1.5fr 1.8fr 0.9fr 1.1fr 0.9fr 0.9fr 90px', padding: '0.75rem 1.5rem', backgroundColor: dark ? '#0D1626' : '#F9FAFB', borderBottom: `1px solid ${cardBorder}` }}>
            ...
            <div style={{ ...thStyle('date'), display: 'flex', alignItems: 'center', gap: 2 }} onClick={() => handleSort('date')}>
              Date{sortIcon('date')}
            </div>
            <div style={{ ...thStyle('amount'), display: 'flex', alignItems: 'center', gap: 2 }} onClick={() => handleSort('amount')}>
              Montant{sortIcon('amount')}
            </div>
            <div style={{ ...thStyle('score'), display: 'flex', alignItems: 'center', gap: 2 }} onClick={() => handleSort('score')}>
              Score IA{sortIcon('score')}
            </div>
          ...
          {filtered.map((claim, i) => {
            const score    = claim.finalScore || 50
            ...
            return (
              <div key={claim.id}
                ...
                <div>
                  <div style={{ fontSize: '0.82rem', color: textMain, fontFamily: 'Helvetica Neue, Arial, sans-serif', fontWeight: 500 }}>{claim.client?.fullName || 'Client'}</div>
                  <div style={{ fontSize: '0.72rem', color: textSub, fontFamily: 'Helvetica Neue, Arial, sans-serif' }}>{claim.client?.company || ''}</div>
                </div>
                <div style={{ fontSize: '0.82rem', color: textBody, fontFamily: 'Helvetica Neue, Arial, sans-serif' }}>{claim.equipment}</div>
                ...
                <div style={{ fontSize: '0.82rem', fontWeight: 600, color: textMain, fontFamily: 'Helvetica Neue, Arial, sans-serif' }}>{claim.amount?.toLocaleString('fr-FR')} DA</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                  ...
                  <span style={{ fontSize: '0.78rem', fontWeight: 700, color: score > 70 ? '#C0392B' : score > 30 ? '#F39C12' : '#1A7A4A', fontFamily: 'Helvetica Neue, Arial, sans-serif', minWidth: 22 }}>{score}</span>
                </div>
                <div>
