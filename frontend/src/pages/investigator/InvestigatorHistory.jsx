 ...
 * InvestigatorHistory
 *
 * FIXED field names to match real backend shape:
 *   claim.equipment        → claim.equipment?.name   (object, not string)
 *   claim.client?.fullName → firstName + lastName    (no fullName field)
 *   claim.amount           → claim.claimedAmount
 *
 * NOTE: GET /claims/flagged only returns HUMAN_REVIEW claims.
 * History of already-decided claims would need a separate backend endpoint.
 * For now we show everything available from flagged (same as dashboard).
 */
...
// FIXED: derive name from firstName + lastName
...
function InvestigatorSidebar({ dark }) {
  ...
  // FIXED: firstName/lastName not fullName
  const invName = clientFullName(user)
    ...
    <div style={{ width, minHeight: '100vh', backgroundColor: bg, display: 'flex', flexDirection: 'column', position: 'fixed', left: 0, top: 0, zIndex: 100, transition: 'width 0.25s', overflow: 'hidden', boxShadow: '4px 0 24px rgba(0,0,0,0.15)' }}>
      ...
      {!collapsed && (
        <div style={{ padding: '1rem 1.5rem', borderBottom: `1px solid ${border}` }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
            {/* FIXED: use initial from firstName */}
...
export default function InvestigatorHistory() {
  ...
  const filtered = claims.filter(claim => {
    ...
    // FIXED: use correct fields for search
          ...
          {filtered.map((claim, i) => {
            ...
            return (
              <div key={claim.id}
                ...
                {/* FIXED: equipment is object { name, type } */}
                <div style={{ fontSize: '0.85rem', color: textMain, fontFamily: 'Helvetica Neue, Arial, sans-serif' }}>
                  {claim.equipment?.name || '—'}
                </div>

                {/* FIXED: client has firstName + lastName, not fullName */}
                <div style={{ fontSize: '0.82rem', color: textSub, fontFamily: 'Helvetica Neue, Arial, sans-serif' }}>
                  {clientFullName(claim.client)}
                </div>

                {/* FIXED: claimedAmount not amount */}
                ...
                {/* Date — use updatedAt as proxy for decision date */}
